...
public class DAOMedConsultarCitas extends ConectorBD{
    ...
    public Cita CompletarCuadros(String ID){
        ...
        Cita cita = null;    
        try{
            rs = st.executeQuery("SELECT CitaDetalles,ID_Mascota,MascotaNombre,MascotaEspecie,MascotaSexo,DueñoDNI,DueñoNombre,DueñoApellido,DueñoCelular from cita where ID_Mascota='"+ID+"';");
            if(rs.next()){
                cita = new Cita();    
                cita.setDetalleCita(rs.getString(1));
                cita.setID_mascota(rs.getString(2));
                cita.setNombreMascosta(rs.getString(3));
                cita.setEspecie(rs.getString(4));
                cita.setSexo(rs.getString(5));
                cita.setDNI(rs.getString(6));
                cita.setNombrePropietario(rs.getString(7));
                cita.setApellido(rs.getString(8));
                cita.setCelular(rs.getString(9));
            }
            rs.close();
        }catch(Exception e){
